...
namespace Tars.Net.Configurations
{
    public class Config
    {
        ...
        public static Config ParseFile(string file, Encoding encode)
        {
            if (File.Exists(file))
                return new Config(File.ReadAllLines(file, encode));
            throw new FileNotFoundException(file);
        }

        public static Config ParseFile(string file)
        {
            return ParseFile(file, Encoding.GetEncoding(Constants.default_charset_name));
        }
    }
}
